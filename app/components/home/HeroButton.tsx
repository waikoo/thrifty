...
export default function HeroButton({ gender, lang }: HeroButtonProps) {
  ...
  return (
    <Link href={`/${lang}/${gender}/products/?gender=${gender}&shop-by=${param}&sort-by=newfirst&page=1`}>
      <button
        className={`absolute left-0 right-0 bottom-10 bg-t_black text-t_white py-[0.5rem] w-[138px] mx-auto rounded-full font-medium text-[0.8125rem] tracking-widest ${albert.className}`}
      >
        SHOP NOW
      </button>
    </Link>
