...
export default function HeroImage({ gender, state }: HeroImageProps) {
  ...
  const height = state === 'new_in' ? 'h-[437px] sm:h-[1039px] xl:h-[35rem] 2xl:h-[48rem] 3xl:h-[55rem]' : 'h-[437px] xl:h-[39.375]'
  const saleBorder = state === 'sale' ? 'border-t_yellow border-[10px] sm:border-[20px]' : ''

  return (
    <div className={`w-full transition flex ${borderRadius} object-cover h-[437px] sm:h-[1039px] xl:h-[35.375rem] 2xl:h-[48.2rem] 3xl:h-[55.2rem] rounded-[1.8rem] relative border-[0.125rem] ${borderColor}`}>
      {state === 'new_in' &&
