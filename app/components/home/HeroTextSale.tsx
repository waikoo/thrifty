...
export default function HeroTextSale({ className }: HeroTextSaleProps) {
  ...
  return (
    <div className={`${className} w-full h-full grid place-items-center`}>
      <div className="relative text-center w-1/2 h-1/2 xl:w-[20rem] xl:h-[20rem]">
        ...
        <span className="absolute left-0 right-0 top-[7rem] sm:top-[13rem] xl:top-[11rem] bottom-16 xl:bottom-20 mx-auto text-[2.5rem] font-noir_pro text-t_green text-shadow-pr shadow-black sm:text-[5rem] xl:text-[3.75rem]">
          %
        </span>

        <span className={`${zen_tokyo_zoo.className} absolute top-[10rem] sm:top-[19rem] xl:top-[16rem] xl:bottom-0 block mx-auto text-[3rem] -rotate-17 text-t_green text-shadow-t_green sm:text-[5.5rem] xl:text-[4.125rem] left-0 right-0`}>
          OFF
        </span>
      </div>
    </div>
